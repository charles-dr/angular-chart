...
export class FormComponent implements OnInit {
  ...
  labels_toShow(): any {
    ...
    for (let i = 0; i < this.RVP_Keys.length; i++) {
      ...
      if (this.HAS_DSPLY[i] === "'Y'") {
        ...
        this.RVP_placeholder.push(
          this.RVP_Keys[i].replace(new RegExp("_", "g"), " ")
        );
      }
  ...
  getOther_records(): any {
    this.V_SRVC_CD = this.PVP["V_SRVC_CD"]
      ? this.PVP["V_SRVC_CD"][0]
      : this.Form_Data["SRVC_CD"][0];
    ...
    this.SL_APP_CD = this.StorageSessionService.getCookies("executedata")[
      "SL_APP_CD"
    ];

    this.SL_PRC_CD = this.StorageSessionService.getCookies("executedata")[
      "SL_PRC_CD"
    ];
  }
  ...
  checkOLD_Table(): any {
    ...
    if ("V_Key_Names" in this.PVP) {
      ...
      const V_KEY_VALUE_arr = this.PVP["V_Key_Values"]
        ? this.PVP["V_Key_Values"]
        : [];
  ...
  invoke_router(res) {
    ...
    if (
      CommonUtils.isValidValue(res["SRVC_CD"]) &&
      res["SRVC_CD"][0] === "END"
    ) {
      ...
      if (timeout && this.ctrl_variables.call_repeat_on_TIMEOUT) {
        ...
        res["RESULT"][0] === "COMPLETED"
      ) {
        this.submitClicked = false;
        this.router.navigate(["/End_User/Design"], {
        ...
        if (res["RESULT"] == "INPUT_ARTFCT_TASK") {
          if (!this.bottomFlag && !this.topFlag) {
          ...
          if (!this.bottomFlag && !this.topFlag) {
            this.showNonRepForm = false;
            this.router.navigate(["/End_User/NonRepeatForm"], {
              skipLocationChange: true,
              queryParams: { page: 1 }
            });
          } else {
            this.showNonRepForm = true;
          }
  ...
  GenerateReportTable() {
    if (!this.app.loadingCharts) this.app.loadingCharts = true;
    if (this.app.fromNonRepForm) {
      this.app.fromNonRepForm = false;
      this.Execute_res_data = this.StorageSessionService.getCookies(
        "executeresdata"
      );
    }
    const body = {
    ...
    this.https
      .post(this.aptUrlPost_report_new, body, this.apiService.setHeaders())
      .subscribe((res: any) => {
        if (res._body !== "{}") {
          this.submitClicked = false;
          ...
          this.StorageSessionService.setCookies("App_Prcs", {
  ...
  getOptional_values(V_PARAM_NM, display_label) {
    const secureUrl =
      ...
      "&V_SRVC_CD=" +
      this.V_SRVC_CD +
      "&REST_Service=ProcessParametersOptions&Verb=GET";
